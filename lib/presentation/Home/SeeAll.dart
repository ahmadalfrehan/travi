...
class SeeAll extends StatelessWidget {
  ...
  const SeeAll({Key? key, required this.category}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: Builder(builder: (context) {
        if (category == 'Offers') {
          TraviCubit.get(context).showOfferedTrips();
        }

        if (category == 'Recommended for you') {
          TraviCubit.get(context).showRecommendedTrips();
        }

        if (category == 'Most popular') {
          TraviCubit.get(context).showBestTrips();
        }
        return BlocConsumer<TraviCubit, TraviStates>(
                      ...
                      child: ListView.separated(
                        itemBuilder: (context, index) => day(
                          ...
                          category == 'Most popular'
                              ? TraviCubit.get(context).best[index]
                              : category == 'Recommended for you'
                                  ? TraviCubit.get(context).recommended[index]
                                  : TraviCubit.get(context).offered[index],
                        ),
                        separatorBuilder: (context, index) =>
                            const SizedBox(height: 15),
                        itemCount: category == 'Most popular'
                            ? TraviCubit.get(context).best.length
                            : category == 'Recommended for you'
                                ? TraviCubit.get(context).recommended.length
                                : TraviCubit.get(context).offered.length,
                      ),
                    )
                  ],
                ),
